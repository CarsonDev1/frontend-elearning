...
import { Camera, CheckCircle, Award, Briefcase, GraduationCap, Plus, Eye, EyeOff, Save, Upload } from 'lucide-react';
...
const ProfilePage: React.FC = () => {
	...
	return (
		<div className='w-full'>
			...
			<div className='mb-8'>
				{user?.roles?.[0] === 'ROLE_TUTOR' && <AnimatedTab activeTab={activeTab} setActiveTab={setActiveTab} />}
			</div>
			...
			<div>
				...
				{activeTab === 2 && (
					<Card className='border-none shadow-md rounded-xl overflow-hidden'>
						...
						<CardContent className='p-4 md:p-6'>
							{user?.educations && Array.isArray(user.educations) && user.educations.length > 0 ? (
								<div className='space-y-4 md:space-y-6'>
									{user.educations.map((edu: Education, index: number) => (
										<div
											key={index}
											className='p-3 md:p-4 border-2 border-gray-100 rounded-xl bg-white hover:border-primary/30 transition-colors'
										>
				...
				{activeTab === 3 && (
					<Card className='border-none shadow-md rounded-xl overflow-hidden'>
						...
						<CardContent className='p-4 md:p-6'>
							{user?.experiences && Array.isArray(user.experiences) && user.experiences.length > 0 ? (
								<div className='space-y-4 md:space-y-6'>
								...
								<div className='py-8 md:py-12 text-center'>
									...
									<Button variant='custom' className='text-sm md:text-base'>
										<Plus className='mr-2 h-3 w-3 md:h-4 md:w-4' />
										Thêm kinh nghiệm
									</Button>
								</div>
